		...
		Validator validator = factory.usingContext()
				/*.messageInterpolator(
		                new ResourceBundleMessageInterpolator(
		                        new PlatformResourceBundleLocator("globalMessages.properties")
		                )
		        )*/
				.traversableResolver( tr )
				.getValidator();
		//logger.info(" DD >> "+factory.usingContext());
		...
		if ( groups.length > 0 ) {
			...
			if ( constraintViolations.size() > 0 ) {
				...
				Set<ConstraintViolation<?>> propagatedViolations =
						new HashSet<ConstraintViolation<?>>( constraintViolations.size() );
				StringBuilder builder = new StringBuilder();
				builder.append( "Validation failed because of : " );
				int i = constraintViolations.size();
				for (ConstraintViolation<?> violation : constraintViolations) {
					builder.append( "'" + violation.getPropertyPath().toString() + "' " + violation.getMessage());
