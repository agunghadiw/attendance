...
public abstract class GenericHibernateDAO<T, ID extends Serializable> implements GenericDAO<T, ID> {
	...
	*/
	...
	@Override
	public List<T> enversById(long id) throws Exception {
		List<T> list = new LinkedList<T>();
		Session session = getSessionAnnotated(getPersistentClass().getName());
		try {
			AuditReader reader = AuditReaderFactory.get(session);
			List<Number> versions = reader.getRevisions(getPersistentClass(), new Long(id));
			for (Number number : versions) {
				T t = (T)reader.find(getPersistentClass(), new Long(id), number);
				list.add(t);
			}
