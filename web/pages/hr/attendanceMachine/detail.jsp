...
<!--sidebar end-->
    ...
    <section id="main-content">
        ...
        <!-- page start-->
			...
			<s:hidden name="attendanceMachineId"/>
			...
			<div class="form-group">
				<label class="col-sm-3 control-label"><s:text name="label.machineId"/></label>
				<div class="col-sm-6">
		    		<s:textfield cssClass="form-control" id="machineId" name="machineId" readonly="%{readOnly}" />
		    	</div>
			</div>
			<div class="form-group">
				<label class="col-sm-3 control-label"><s:text name="label.branch"/></label>
				<div class="col-sm-7">
		    		<s:optiontransferselect cssClass="form-control" cssStyle="overflow-x:scroll;width:300px;height:300px;overflow: -moz-scrollbars-horizontal;" doubleCssClass="form-control" doubleCssStyle="overflow-x:scroll;width:300px;height:300px;overflow: -moz-scrollbars-horizontal;" size="20"  leftTitle="Available" rightTitle="Choosen" name="availableBranchIds" list="availableBranchs" listKey="branchId" listValue="name" multiple="true" doubleName="choosenBranchIds" doubleList="choosenBranchs" doubleListKey="branchId" doubleListValue="name" disabled="%{readOnly}" doubleDisabled="%{readOnly}"/>
		    	</div>
			</div>
			<div class="form-group">
				<label class="col-sm-3 control-label"><s:text name="label.isRestart"/></label>
				<div class="col-sm-6">
		    		<s:checkbox name="restart" disabled="%{readOnly}" />&nbsp;<s:text name="label.yes"/>
		    	</div>
			</div>
			<div class="form-group">
				<label class="col-sm-3 control-label"><s:text name="label.lastResponse"/></label>
				<div class="col-sm-3">
		    		<s:textfield cssClass="form-control" id="lastResponse" name="lastResponse" readonly="%{readOnly}" />
		    	</div>
			</div>
			<div class="form-group">
				<label class="col-sm-3 control-label"><s:text name="label.lastMessage"/></label>
				<div class="col-sm-6">
		    		<s:textfield cssClass="form-control" id="lastMessage" name="lastMessage" readonly="%{readOnly}" />
		    	</div>
			</div>
			<div class="form-group">
				<label class="col-sm-3 control-label"><s:text name="label.createBy"/></label>
				<div class="col-sm-5">
		    		<s:textfield cssClass="form-control" name="attendanceMachine.createBy" readonly="%{readOnly}" />
		    	</div>
			</div>
			<div class="form-group">
				<label class="col-sm-3 control-label"><s:text name="label.createOn"/></label>
				<div class="col-sm-5">
		    		<s:textfield cssClass="form-control" name="attendanceMachine.createOn" readonly="%{readOnly}" />
		    	</div>
			</div>
			<div class="form-group">
				<label class="col-sm-3 control-label"><s:text name="label.changeBy"/></label>
				<div class="col-sm-5">
		    		<s:textfield cssClass="form-control" name="attendanceMachine.changeBy" readonly="%{readOnly}" />
		    	</div>
			</div>
			<div class="form-group">
				<label  class="col-sm-3 control-label"><s:text name="label.changeOn"/></label>
				<div class="col-sm-5">
		    		<s:textfield cssClass="form-control" name="attendanceMachine.changeOn" readonly="%{readOnly}" />
		    	</div>
			</div>
			<div class="form-group">
				<div class="col-sm-offset-3 col-sm-5">
		    		<input type="button" class="btn btn-sml" value="<s:text name="label.edit.link"/>" onclick="window.location.href=('<s:url value="AttendanceMachineAction_edit.action"/>?attendanceMachineId=<s:property value="attendanceMachineId"/>');"/>&nbsp;
					<input type="button" class="btn btn-sml" value="<s:text name="label.delete.link"/>" onclick="window.location.href=('<s:url value="AttendanceMachineAction_delete.action"/>?attendanceMachineId=<s:property value="attendanceMachineId"/>');"/>&nbsp;
					<input type="button" class="btn btn-sml" value="<s:text name="label.back.link"/>" onclick="window.location.href=('<s:url value="AttendanceMachineAction_partialList.action"/>');"/>
		    	</div>
			</div>
  			</s:form>
