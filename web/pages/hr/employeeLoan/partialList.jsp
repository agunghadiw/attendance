        ...
        <!-- page start-->
			...
			<s:token></s:token>
		  	<div class="form-group">
			    <label class="col-sm-2 control-label"><s:text name="label.loanDate"/></label>
			    <div class="col-sm-3">
			    	<div class="input-group">
			      	<s:textfield cssClass="datePicker form-control" id="fromLoanDate" name="fromLoanDateX" readonly="%{readOnly}" placeholder="dd mmm yyyy"/><label for="date-picker-2" class="input-group-addon btn"><span class="glyphicon glyphicon-calendar"></span></label>
			    	</div>
			    </div>
			    <div class="col-sm-3">
			    	<div class="input-group">
			      	<s:textfield cssClass="datePicker form-control" id="toLoanDate" name="toLoanDateX" readonly="%{readOnly}" placeholder="dd mmm yyyy"/><label for="date-picker-2" class="input-group-addon btn"><span class="glyphicon glyphicon-calendar"></span></label>
			    	</div>
			    </div>
				</div>
		  	<div class="form-group">
			    <label class="col-sm-2 control-label"><s:text name="label.employeeNumber"/></label>
			    <div class="col-sm-3">
			...
			<table class="table table-bordered table-striped">
				<thead>
			        <tr>
			          ...
			          <th><s:text name="label.branch"/></th>
			          <th><s:text name="label.loanDate"/></th>
			          <th><s:text name="label.loanAmount"/></th>
			          <th><s:text name="label.duration"/></th>
			          <th><s:text name="label.installment"/></th>
			          <th><s:text name="label.terminateDate"/></th>
			          <th><s:text name="label.remainingLoan"/></th>
			          <th>&nbsp;</th>
			        </tr>
		        </thead>
        <%
...
%>
         		<tbody>
		            <s:iterator value="employeeLoanLists" status="sequence">
		            <tr>
			            <td><%=++i%>.</td>
			            <td><s:property value="number"/></td>
			            <td><s:property value="fullName"/></td>
			            <td><s:property value="branch"/></td>
			            <td><s:date name="loanDate" format="dd MMM yyyy"/></td>
			            <td align="right"><s:property value="%{getText('format.number',{loanAmount})}" /></td>
			            <td><s:property value="duration"/></td>
			            <td align="right"><s:property value="%{getText('format.number',{installment})}" /></td>
			            <td><s:date name="terminateDate" format="dd MMM yyyy"/></td>
			            <td align="right"><s:property value="%{getText('format.number',{remainingLoan})}" /></td>
			            <td>
			            	<a href="<s:url value="EmployeeLoanAction_detail.action"/>?employeeLoanId=<s:property value="employeeLoanId"/>"><i class="fa fa-search-minus" title="Detail"></i></a>&nbsp;
		                <s:if test="(remainingLoan==loanAmount)">
		                <a href="<s:url value="EmployeeLoanAction_edit.action"/>?employeeLoanId=<s:property value="employeeLoanId"/>"><i class="fa fa-pencil" title="Edit"></i></a>
			            	</s:if>
			            	<s:if test="(terminateDate==null)">
			            	<a href="<s:url value="EmployeeLoanAction_terminateAdd.action"/>?employeeLoanId=<s:property value="employeeLoanId"/>"><i class="fa fa-credit-card" title="Payment"></i></a>
			            	</s:if>
			            </td>
		            </tr>
		            </s:iterator>
  			...
  			<div class="col-sm-offset-6 col-sm-6">
		     	<input class="btn btn-sml" type="button" onclick="window.location.href=('<s:url value="EmployeeLoanAction_add.action"/>');" value="<s:text name="label.new.link"/>">
		  	</div>
