        ...
        <!-- page start-->
			...
			<s:token></s:token>
		  	<div class="form-group">
				<label class="col-sm-3 control-label"><s:text name="label.roleName"/></label>
				<div class="col-sm-5">
		    		<s:textfield cssClass="form-control" name="roleName" readonly="%{readOnly}" />
		    	</div>
			</div>
			<div class="form-group">
				<label class="col-sm-3 control-label"><s:text name="label.userGroup"/></label>
				<div class="col-sm-5">
		    		<s:select cssClass="form-control" list="userGroupList" name="userGroupId" listKey="lookupId" listValue="name" headerKey="" headerValue="Select bellow" disabled="%{readOnly}"/>
		    	</div>
			</div>
			<div class="form-group">
				<label class="col-sm-3 control-label"><s:text name="label.permission"/></label>
				<div class="col-sm-9">
					<s:optiontransferselect cssClass="form-control" cssStyle="overflow-x:scroll;width:250px;height:300px;overflow: -moz-scrollbars-horizontal;" doubleCssClass="form-control" doubleCssStyle="overflow-x:scroll;width:250px;height:300px;overflow: -moz-scrollbars-horizontal;" size="20" leftTitle="Available" rightTitle="Choosen" name="availablePermissionIds" list="availablePermissionList" listKey="permissionId" listValue="parentPermissionName" multiple="true" doubleName="choosenPermissionIds" doubleList="choosenPermissionList" doubleListKey="permissionId" doubleListValue="parentPermissionName" disabled="%{readOnly}" doubleDisabled="%{readOnly}"/>
				</div>
			</div>
			<div class="form-group">
				<div class="col-sm-offset-3 col-sm-5">
					<input type="button" class="btn btn-sml" value="<s:text name="label.edit.link"/>" onclick="window.location.href=('<s:url value="/basic/RoleAction_edit.action"/>?roleId=<s:property value="roleId"/>');"/>&nbsp;
					<input type="button" class="btn btn-sml" value="<s:text name="label.delete.link"/>" onclick="window.location.href=('<s:url value="/basic/RoleAction_delete.action"/>?roleId=<s:property value="roleId"/>');"/>&nbsp;
					<input type="button" class="btn btn-sml" value="<s:text name="label.back.link"/>" onclick="window.location.href=('<s:url value="/basic/RoleAction_partialList.action"/>');"/>
				</div>
			</div>
  			</s:form>
